...
async def publish(df):
  ...
  if knack:
    ...
    await knack.update(objectName='LaatsteUpdate', df=pd.DataFrame([{'Key': 1, **{ f'Timestamp RegioV2 {provider.upper()}': ts for provider, ts in Cache.timestamps.items() if provider != 'github' }}]))
